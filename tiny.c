...
void client_error(int fd, int status, char *msg, char *longmsg)
{
    ...
    snprintf(buf + strlen(buf), MAXLINE, "Content-length: %lu\r\n\r\n",
             strlen(longmsg));
...
void serve_static(int out_fd, int in_fd, http_request *req, size_t total_size)
{
    ...
    if (req->offset > 0) {
        ...
        snprintf(buf + strlen(buf), 256, "Content-Range: bytes %lu-%lu/%lu\r\n",
...
int __handle_request(int fd, http_request *req)
{
...
int handle_request(int fd, http_request *req)
{
    int status = 200;
    char buf[MAXLINE];
    snprintf(buf, MAXLINE, "HTTP/1.1 200 OK\r\n%s%s%s%s%s%s",
